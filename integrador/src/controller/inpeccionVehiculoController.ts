...
export const getInspeccionVehiculoById = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const inspeccion = await InspeccionVehiculo.findById(id)
      .populate('id_recibo')
...
export const getAllInspeccionVehiculo = async (req: Request, res: Response) => {
  try {
    const inspecciones = await InspeccionVehiculo.find()
      .populate('id_recibo')
      .populate('id_empleado');
...
export const getPaginatedInspeccionVehiculo = async (req: Request, res: Response) => {
  try {
    ...
    return res.status(500).json({ 
      message: 'Error al obtener inspecciones paginadas', 
      error: error instanceof Error ? error.message : error 
    });
  }
};
...
export const updateInspeccionVehiculo = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const inspeccionActualizada = await InspeccionVehiculo.findByIdAndUpdate(
      id,
      req.body,
      { new: true }
    ).populate('id_recibo')
