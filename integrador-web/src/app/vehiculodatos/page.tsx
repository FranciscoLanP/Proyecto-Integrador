...
export default function VehiculoDatosPage() {
  ...
  if (vehCrud.allQuery.isLoading) return <Typography>Loading…</Typography>
  if (vehCrud.allQuery.error)     return <Typography color="error">{vehCrud.allQuery.error.message}</Typography>
  ...
  const onSubmit = (data: Partial<IVehiculoDatos>) => {
    if (editData) {
      vehCrud.updateM.mutate({ id: editData._id, data })
    } else {
      vehCrud.createM.mutate(data)
    }
    closeForm()
  }
  ...
  // Alternar visibilidad (campo activo)
    ...
    <Box p={3}>
      ...
      <Paper elevation={1} sx={{ borderRadius: 2, overflow: 'hidden' }}>
        <Table size="small">
          ...
          <TableBody>
            {paginated.map(v => {
              const cliente = clientes.find(c => c._id === v.id_cliente)?.nombre      ?? '—'
              const modelo  = modelos.find(m  => m._id === v.id_modelo )?.nombre_modelo ?? '—'
              const color   = colores.find(c  => c._id === v.id_color  )?.nombre_color  ?? '—'
